...
export default function CustomOrderPage() {
	...
	const [items, setItems] = useState<Array<{ id: string; name: string; size: string; material: string; quantity: number; images: File[] }>>([
		{ id: uuid(), name: '', size: '', material: '', quantity: 1, images: [] },
	])

	const addRow = () => setItems(prev => [...prev, { id: uuid(), name: '', size: '', material: '', quantity: 1, images: [] }])
	const removeRow = (id: string) => setItems(prev => prev.filter(i => i.id !== id))

	const update = (id: string, patch: Partial<{ name: string; size: string; material: string; quantity: number; images: File[] }>) =>
				...
				{items.map((row) => (
					<div key={row.id} className="p-4 border rounded-md bg-white">
						<div className="grid md:grid-cols-5 gap-3">
							...
							<input className="input-field" placeholder="Custom size" value={row.size} onChange={(e)=>update(row.id,{size:e.target.value})} />
